...
const Sidebar = () => {
  ...
  const SubmitHandler = async (e) => {
    e.preventDefault();
    const SearchRes = otherUser?.find((user) =>
  ...
  const handleClick = (user) => {
    dispatch(setSelectedUser({ ...user, updatedAt: true }));
  ...
  return (
    <div
      className={`${
        selectedUser.updatedAt ? "hidden" : "flex"
      } w-2/5 sm:max-w-1/2 border-slate-400 border-r-2 p-4  flex-col`}
    >
      <form action="" onSubmit={SubmitHandler} className="flex items-center ">
        <div className="relative w-full">
          ...
          <button
            type="submit"
            className="btn btn-circle absolute right-0.5 text-white bg-gray-300"
          >
            <MdOutlinePersonSearch size="25px" />
          </button>
        </div>
      </form>
